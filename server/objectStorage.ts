...
export class ObjectStorageService {
  ...
  async searchPublicObject(filePath: string): Promise<StorageFile | null> {
    ...
    for (const searchPath of this.getPublicObjectSearchPaths()) {
      ...
      const { bucketName, objectName } = provider.parsePath(fullPath);
      const file = provider.getFile(bucketName, objectName);
  ...
  async getObjectEntityUploadURL(): Promise<string> {
    ...
    return provider.getSignedUrl(bucketName, objectName, {
      method: "PUT",
  ...
  async getObjectEntityFile(objectPath: string): Promise<StorageFile> {
    ...
    const { bucketName, objectName } = provider.parsePath(objectEntityPath);
    const objectFile = provider.getFile(bucketName, objectName);
  ...
  normalizeObjectEntityPath(
    ...
    // Check for cloud storage URLs (GCS, S3, Azure)
